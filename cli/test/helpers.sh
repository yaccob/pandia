#!/usr/bin/env bash
# -------------------------------------------------------------------
# CLI test helpers — assertions and utilities
# Sourced by CLI test modules; not meant to be run directly.
# -------------------------------------------------------------------

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CLI_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PROJECT_DIR="$(cd "$CLI_DIR/.." && pwd)"
PANDIA="$CLI_DIR/bin/pandia"
FILTER="$PROJECT_DIR/server/diagram-filter.lua"
